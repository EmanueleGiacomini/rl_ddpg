...
class Actor(object):
    def __init__(self, state_space, action_space, lr):
        def create_actor_network() -> tf.keras.Model:
            ...
            model.add(Dense(10, activation='relu', input_dim=state_space, name='state_input'))
            #model.add(BatchNormalization())
            model.add(Dense(5, activation='relu', name='state_dense1'))
            model.add(Dense(action_space, activation='tanh',
                            kernel_initializer=RandomUniform(-0.07, 0.07)
                            ))
